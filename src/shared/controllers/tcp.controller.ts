...
export class TcpController {
  ...
  async createClient(@Param('name') name: string, @Body() config: CreateTcpClientDto) {
    await this.tcpService.createClient(name, config);
  ...
  async connectClient(@Param('name') name: string, @Body() config: ConnectClientDto) {
    await this.tcpService.connectClient(name, config);
  ...
  @ApiResponse({ status: 200, description: '消息发送成功' })
  async sendToConnection(@Param('connectionId') connectionId: string, @Body() messageDto: SendToConnectionDto) {
